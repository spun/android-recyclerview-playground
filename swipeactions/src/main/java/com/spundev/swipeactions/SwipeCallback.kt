...
class SwipeCallback<T>(
    ...
    override fun onChildDraw(
        ...
        // onChildDraw is being called after the item is fully dismissed. When this happens, the
        // adapter positions is -1. To avoid extra work, we skip the drawing if we receive a -1
        if (viewHolder.bindingAdapterPosition != -1) {
            ...
            if (dX > 0) { // Swiping from left to right
                ...
                if (swipeRightIcon != null) {
                    ...
                    val iconRight =
                        itemView.left + easeOutMargin + swipeRightIcon.intrinsicWidth
